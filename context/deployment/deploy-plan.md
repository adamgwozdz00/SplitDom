...
3. **Open issue, not yet fixed**: Supabase's default email-confirmation flow uses PKCE, which requires the `code_verifier` (stored in a cookie set during `signUp`) to be present when the confirmation link is opened. If a user signs up in one browser/device and opens the confirmation email in another (a very plausible pattern for a household-expense app — phone Gmail app vs. desktop browser), `exchangeCodeForSession` fails with "PKCE code verifier not found in storage". This is a product/UX decision (PKCE vs. implicit flow for email confirmations, or an explicit "resend/re-request" recovery path), not a deploy-pipeline defect — tracked as a follow-up, not blocking the deployment itself.
